...
impl Element {
    ...
    pub fn read_from<R>(reader: &mut Reader<R>) -> SCResult<Element>
    where
        R: BufRead,
    {
    ...
    pub fn write_to<W>(&self, writer: &mut Writer<W>) -> SCResult<()>
    where
        W: Write,
    {
    ...
    fn write_to_impl<W>(&self, writer: &mut Writer<W>) -> SCResult<()>
    where
        W: Write,
    {
    ...
    pub fn attribute(&self, key: &str) -> SCResult<&str> {
        self.attributes.get(key).map(|s| s.as_str()).ok_or_else(|| {
            format!("No attribute with key '{}' found in <{}>!", key, self.name).into()
        })
    }
    ...
    pub fn child_by_name<'a, 'n: 'a>(&'a self, name: &'n str) -> SCResult<&'a Element> {
        self.childs_by_name(name)
            .next()
            .ok_or_else(|| format!("No <{}> found in <{}>!", name, self.name).into())
    }
...
impl<'a> From<ElementBuilder<'a>> for Element {
    fn from(builder: ElementBuilder<'a>) -> Self {
        builder.build()
    }
}
...
impl<'a> From<&'a Element> for BytesStart<'a> {
    fn from(element: &'a Element) -> Self {
        BytesStart::borrowed_name(element.name.as_bytes()).with_attributes(
            element.attributes.iter().map(|(k, v)| Attribute {
                key: k.as_bytes(),
...
mod tests {
    ...
    fn test_read() {
        assert_eq!(
            "<Test/>".parse::<Element>().unwrap(),
            Element::new("Test").build()
        );
    }
}
