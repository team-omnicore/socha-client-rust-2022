...
use socha_client_2022::client::{DebugMode, SCClient};
...
fn main() {
    ...
    options.optopt(
        "l",
        "level",
        "Optionally provides a custom log level ('Info' by default)",
        "LEVEL",
    );
    options.optflag(
        "d",
    ...
    options.optflag(
        "D",
        "debug-writer",
        "Prints incoming XML messages to the console for debugging",
    );
    ...
    // Setup logging
    SimpleLogger::init(
        LevelFilter::from_str(&level).expect("Invalid log level."),
        Config::default(),
    )
